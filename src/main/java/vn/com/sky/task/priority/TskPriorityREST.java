...
@Configuration
@AllArgsConstructor
public class TskPriorityREST extends GenericREST {
    private TskPriorityRepo mainRepo;

    @Bean
    public RouterFunction<?> tskPriorityRoutes() {
        return route(POST(buildURL(Constants.API_TASK_PREFIX, "priority", this::saveOrUpdate)), this::saveOrUpdate);
    }
    
    private Mono<List<LinkedHashMap<String, String>>> validateForSave(TskPriority req) {
    ...
    private Mono<List<LinkedHashMap<String, String>>> validateForUpdate(TskPriority req) {
    	var validateCode = utilRepo
                                ...
                                .flatMap(
                                    errs -> {
                                        if (errs.size() > 0) {
                                            ...
                                            return updateEntity(mainRepo, req, getUserId(request)).flatMap(e -> ok(e, TskPriority.class));
                                        }
                                    }
                                );
                        }
